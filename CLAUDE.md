...
- `2026-05-13` — Task #73 (Multi-admin) concluída. `isActive` + `mustChangePassword` no modelo `User`. Migration `20260513240000_add_user_admin_flags`. Prisma regenerado. API: `GET/POST/PATCH/DELETE /admin/users`, `POST /auth/admin/change-password` (bcrypt 12, seta `mustChangePassword=false`). `AdminUser` type atualizado + `AdminUserListItem`. Frontend: modal forçado de troca de senha no primeiro acesso (`AdminShell`), "Alterar senha" no user menu, página `/admin/configuracoes/usuarios` com tabela CRUD + modais criar/editar. Senha temporária exibida **uma única vez** com botão copiar. Build + typecheck + lint: OK.

- `2026-05-13` — Task #74 (CMS home) concluída. `HomePageContent` singleton Prisma. API: `GET /home-content` público, `PUT /home-content/admin` e `POST /home-content/admin/banner-image` (Sharp WebP, R2). Frontend: `/admin/configuracoes/aparencia` com 3 seções independentes (banner c/ upload drag-drop + XHR progress, sobre nós, contato). Loja: home e footer consomem CMS (ISR 60s, fallback para defaults). `next/image` configurado para `*.r2.dev`.
