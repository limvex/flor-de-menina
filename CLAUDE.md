...
| 23  | Dashboard admin                       | -                                           | -                               | -   |
| 24  | Páginas institucionais e SEO          | ⏳ Em progresso (branch local)              | `feat/24-institucionais-seo`    | -   |
...
- `2026-05-12` — Task #24: módulo `pages` na API (`GET/POST/PATCH/DELETE /admin/pages`, `PATCH .../toggle-active`, público `GET /pages` e `GET /pages/:slug`). Prisma `InstitutionalPage` com `metaTitle`/`metaDescription` (@map seo\*), `ogImage`, `sortOrder`, índice `isActive`. Seed HTML em `prisma/institutional-pages-seed.ts`. Admin `/admin/paginas` com TipTap + preview SEO. Loja `/p/[slug]` ISR 60s, sitemap/robots/metadataBase, JSON-LD Organization no layout, GTM/Pixel opcionais (`NEXT_PUBLIC_GTM_ID`, `NEXT_PUBLIC_META_PIXEL_ID`). Footer institucional → `/p/...`. `/quem-somos` → redirect 308 para `/p/sobre`. PDP: Product schema com preço mínimo de variante + descrição texto. Migration: `20260512130000_institutional_page_og_sort_index`.
...
- **Páginas institucionais**: API pública `GET /pages` (resumo) e `GET /pages/:slug` (HTML TipTap). Conteúdo só em `dangerouslySetInnerHTML` na loja. Admin em `/admin/paginas`. ISR `revalidate = 60` em `/p/[slug]`.
